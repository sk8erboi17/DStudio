...
try {
  ...
  console.log('ui_attachment_preview_playwright_test: playwright missing, skipping');
  process.exit(0);
}
...
try {
  ...
  server.close();
  console.log('ui_attachment_preview_playwright_test: browser missing, skipping');
  process.exit(0);
}
...
try {
  ...
  await page.addInitScript(({ filename, preview }) => {
    const now = Date.now();
