...
def main() -> None:
    ...
    assert worker.H3_PATCH_FILE.is_file()
    assert worker.H3_PATCH_SCRIPT.is_file()
    assert len(worker.h3_patch_sha256()) == 64
    assert worker.h3_runtime_revision() == (
        f"{worker.H3_COMMIT}+{worker.h3_patch_sha256()}"
    )
    ...
    assert worker.TRANSFORMER_COPY_HEADROOM_BYTES == 24 * 2**30
    assert worker.TRANSFORMER_COPY_MINIMUM_MEMORY_BYTES == (
        worker.REFERENCE_MODEL_TOTAL_BYTES + 24 * 2**30
    )
    assert worker.BOUNDED_DIT_COMMAND_BLOCKS == "1"
    assert worker.QUALITY_DIT_STAGE_SUBMITS == "1"
    assert worker.QUALITY_SDPA_QUERY_CHUNK == "8"
    assert worker.NATIVE_STATUS_HEARTBEAT_SECONDS == 30.0
    assert worker.POWER_ASSERTION_MODE == "caffeinate-idle-system-sleep"
    ...
    with tempfile.TemporaryDirectory(prefix="dstudio-h3-native-checkout-") as temporary:
        ...
        assert '"schema": "dstudio.h3.failure.v1"' in worker_source
        assert 'native_log_path = output_dir / "h3-native.log"' in worker_source
        assert '"lastProgress": current_payload' in worker_source
