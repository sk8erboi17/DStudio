...
grep -q 'onboardedVersion !== 6' "${tmp}/loading.html"
...
NODE
  ...
  if [ "${lan_loading_code}" = "200" ]; then
    echo "expected LAN GET /loading.html to stay host-only, got 200"
    exit 1
  fi
  if [ "${lan_loading_code}" != "000" ] && [ "${lan_loading_code}" != "403" ]; then
    echo "expected LAN GET /loading.html to be 403 or unreachable in CI, got ${lan_loading_code}"
    exit 1
  fi
