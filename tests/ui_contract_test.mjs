...
assert.match(windowsBuild, /mingw-w64-ucrt-x86_64-gcc/, 'Windows build should install the native UCRT GCC, not rely on MSYS gcc');
assert.match(windowsBuild, /curl\.exe/, 'Windows package should include curl.exe for LAN client remote model calls');
...
assert.match(launcher, /DS4UI_CURL/, 'Windows launcher should point remote model helpers at the prepared curl.exe');
...
assert.doesNotMatch(remoteHelper, /popen\(cmd\.ptr/, 'remote model helper must not use popen for LAN model calls');
assert.match(remoteHelper, /execvp\(argv\[0\], argv\)/, 'remote model helper should exec curl directly on POSIX/MSYS without /bin/sh');
assert.match(remoteHelper, /CreateProcessA\(NULL, cmd\.ptr/, 'remote model helper should launch curl directly on native Windows');
