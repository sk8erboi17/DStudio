...
if [ "$(uname -s)" != "Darwin" ]; then
    echo "h3_sdpa_query_chunk_equivalence: skipped (Metal requires macOS)"
    exit 0
fi
...
h3_test_source=${h3_test_root}/tests/h3_sdpa_query_chunk_equivalence.c

for h3_required in git clang; do
    if ! command -v "$h3_required" >/dev/null 2>&1; then
        echo "h3_sdpa_query_chunk_equivalence: skipped ($h3_required missing)"
        exit 0
    fi
done

if [ ! -d "$h3_checkout/.git" ]; then
    echo "h3_sdpa_query_chunk_equivalence: skipped (managed H3 checkout missing)"
    exit 0
fi
