#!/bin/sh
# DStudio — install the local embedding runtime (llama.cpp, cross-platform, no python).
# Called by POST /api/embed/setup. Downloads a pinned llama.cpp prebuilt release
# for this OS/arch and extracts llama-server. The selected embedding GGUF is fetched lazily by llama-server's -hf on first run (see embed-server.sh).
#
...
case "$os" in
  Darwin)
    case "$arch" in
      ...
      *) echo "embed-setup: unsupported macOS arch: $arch" >&2; exit 2 ;;
    esac ;;
  Linux)
    case "$arch" in
      x86_64|amd64)
      ...
      *) echo "embed-setup: unsupported Linux arch: $arch (need x86_64)" >&2; exit 2 ;;
    esac ;;
  *) echo "embed-setup: $os is not supported yet (Windows: follow-up)" >&2; exit 2 ;;
esac
...
for asset in $assets; do
  echo "embed-setup: downloading $asset" >&2
  if curl -fL --retry 3 -o "$tmp/llama.tgz" "$BASE/$asset"; then
    got="$asset"
    break
  fi
  echo "embed-setup: $asset unavailable, trying fallback" >&2
done
[ -n "$got" ] || { echo "embed-setup: no llama.cpp asset could be downloaded" >&2; exit 3; }

echo "embed-setup: extracting $got into $DIR" >&2
tar -xzf "$tmp/llama.tgz" -C "$DIR"

SERVER="$(find "$DIR" -name llama-server -type f 2>/dev/null | head -1 || true)"
[ -n "$SERVER" ] || { echo "embed-setup: llama-server not found after extraction" >&2; exit 3; }
...
echo "embed-setup: llama-server ready at $SERVER" >&2
