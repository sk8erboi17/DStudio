...
router.get('/ssrf-boundary/summary', async (req, res) => {
...
router.get('/ssrf-boundary/records/:id', async (req, res) => {
  const record = await records.findForUser(req.params.id, req.user);
  if (!record) return res.status(404).json({ error: 'not found' });
  return res.json(record);
});

router.post('/ssrf-boundary/export', requireRole('admin'), async (_req, res) => {
