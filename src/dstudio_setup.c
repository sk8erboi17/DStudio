...
/* Compatibility endpoint: validate the bundle without fetching or overwriting. */
static void api_setup_content(int fd) {
    if (content_present()) {
        send_json(fd, "200 OK", "{\"ok\":true,\"already\":true,\"contentOk\":true,\"bundled\":true}");
    } else {
        send_json(fd, "409 Conflict", "{\"ok\":false,\"contentOk\":false,\"bundled\":true,\"error\":\"Bundled DStudio originals are missing or incomplete. Rebuild or reinstall DStudio; no external catalog is downloaded.\"}");
    }
}
