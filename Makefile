...
ifeq ($(UNAME),Darwin)
  APPCXX       := clang++
  APP_CXXFLAGS := -x objective-c++ -fno-objc-arc -std=c++11 -O2 -Wall
  ...
  APPCXX       := $(CXX)
  APP_CXXFLAGS := -x c++ -std=c++11 -O2 -Wall $(shell pkg-config --cflags gtk+-3.0 webkit2gtk-4.1)
...
# tr -d '\n' → pure base64 stream; fold → short lines; sed → adjacent C literals
# (the compiler concatenates them). Regenerated on every change to index.html.
$(GEN): $(PAGE)
	@{ \
