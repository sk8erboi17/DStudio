...
test-ui-browser:
	@if command -v node >/dev/null 2>&1; then node tests/ui_loading_playwright_test.mjs && node tests/ui_agent_design_playwright_test.mjs && node tests/ui_gear_popover_test.mjs && node tests/ui_attachment_preview_playwright_test.mjs; else echo "node missing: skipping UI browser tests"; fi
